...
class InterviewsResults extends React.Component {
        ...
        api.get(`interviews/results/?user_id=${this.state.user_id}&survey_id=${this.state.survey_id}`)
            .then(res => {
                if (res.data) {
                    ...
                    this.setState({
                        dataset: [...this.state.dataset, ...[{ data: emptyData, meta: { color: 'white' } }, { data: selfs, meta: { color: 'red' } }, { data: colleagues, meta: { color: 'blue' } }, { data: company, meta: { color: 'yellow' } }]]
                    })
                }
            })
    ...
    handleColleagues = () => {
        ...
        let colleagues_filled = new SpecialArray;
        colleagues_filled.push(categories).push(colleagues).toArray()
        const colleagues_dataset = { data: colleagues, meta: { color: 'blue' } };

        if (!this.state.isActivatedColleagues) {
            this.setState({
                dataset: [...this.state.dataset, colleagues_dataset]
            })
        } else {
            this.setState({
                dataset: this.state.dataset.filter(function (item) {
                    return item.meta.color !== 'blue';
                })
            })
        }
        this.setState(state => ({
    ...
    handleCompany = () => {
        ...
        let company_filled = new SpecialArray;
        company_filled.push(categories).push(company).toArray()
        const company_dataset = { data: company, meta: { color: 'yellow' } };

        if (!this.state.isActivatedCompany) {
            this.setState({
                dataset: [...this.state.dataset, company_dataset]
            })
        } else {
            this.setState({
                dataset: this.state.dataset.filter(function (item) {
                    return item.meta.color !== 'yellow';
                })
            })
        }
        this.setState(state => ({
                    ...
                    <div className='col comments-col'>
                        ...
                        <ul className='mt-3'>
                            {this.state.comments && this.state.comments.map((item, i) =>
                                <div className='d-flex mt-3' key={i}><img src='../../../icons/quotes.svg' className='align-self-center' /><li key={i}>{item}</li></div>
                            )}
                        </ul>
                    </div>
                </div>
